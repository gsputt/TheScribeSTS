...
public class AnomalousTextsVariable extends DynamicVariable {
    @Override
    public String key() {
        return "ANOMALOUSTEXTSVARIABLE";
    ...
    public boolean isModified(AbstractCard card) {
        if (card instanceof AbstractScribeCard) {
            ...
            return card1.isAnomalousTextsVariableModified;
        } else {
            return false;
        }
    ...
    public int value(AbstractCard card) {
        if (card instanceof AbstractScribeCard) {
            AbstractScribeCard card1 = (AbstractScribeCard) card;
            return card1.anomalousTextsVariable * 25;
        } else {
            return 0;
        }
    ...
    public boolean upgraded(AbstractCard card) {
        if(card instanceof AbstractScribeCard) {
            AbstractScribeCard card1 = (AbstractScribeCard) card;
            return card1.upgradeAnomalousTextsVariable;
        }
        else
        {
            return false;
