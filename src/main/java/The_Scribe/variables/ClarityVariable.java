...
{   // Custom Dynamic Variables are what you do if you need your card text to display a cool, changing number that the base game doesn't provide.
    ...
    public int value(AbstractCard card)
    {
        if(AbstractDungeon.player != null) {
            if (AbstractDungeon.player.hasPower(SpellClarity.POWER_ID) && AbstractDungeon.player.hasPower(SpellEffectiveness.POWER_ID)) {
                return (int) Math.ceil(AbstractDungeon.player.getPower(SpellClarity.POWER_ID).amount
                        * (1 + (AbstractDungeon.player.getPower(SpellEffectiveness.POWER_ID).amount * 0.01 * 25)));
            } else if (AbstractDungeon.player.hasPower(SpellClarity.POWER_ID)) {
                return (int) Math.ceil(AbstractDungeon.player.getPower(SpellClarity.POWER_ID).amount);
            } else {
                return 0;
            }
