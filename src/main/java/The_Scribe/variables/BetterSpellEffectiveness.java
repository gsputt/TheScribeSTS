...
//Conversion
//AmplifyingSpellstone
//FocusingBlow

public class BetterSpellEffectiveness extends DynamicVariable {
    @Override
    public String key() {
        return "SPELLEFFECTIVENESS";
    ...
    public boolean isModified(AbstractCard card) {
        if (card instanceof AbstractScribeCard) {
            ...
            return card1.isEffectivityModified;
        } else {
            return false;
        }
    ...
    public boolean upgraded(AbstractCard card) {
        if(card instanceof AbstractScribeCard) {
            AbstractScribeCard card1 = (AbstractScribeCard) card;
            return card1.upgradeEffectivity;
        }
        else
        {
            return false;
