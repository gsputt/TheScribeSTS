...
public class BetterSelfDamage extends DynamicVariable {
    @Override
    public String key() {
        return "BETTERSELFDAMAGE";
    ...
    public boolean isModified(AbstractCard card) {
        if (card instanceof AbstractScribeCard) {
            ...
            return card1.isSelfDamageModified;
        } else {
            return false;
        }
    ...
    public boolean upgraded(AbstractCard card) {
        if(card instanceof AbstractScribeCard) {
            AbstractScribeCard card1 = (AbstractScribeCard) card;
            return card1.upgradeSelfDamage;
        }
        else
        {
            return false;
